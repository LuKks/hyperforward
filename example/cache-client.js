...
async function main () {
  ...
  for (let i = 0; i < 3; i++) {
    const socket = node.connect(keyPairServer.publicKey)
    socket.write(Buffer.from('hey'))

    const time = Date.now()
    await waitForEvent('open', socket)
    console.log('opened in', Date.now() - time, 'ms', 'is utp?', !!socket._utp)
  }
